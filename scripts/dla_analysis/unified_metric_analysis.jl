...
PLOTSDIR = mkpath(plotsdir("dla_analysis", "unified", WITH_REMITTANCES ? "with_rem_gdp" : "without_rem_gdp"))
...
const obs_mean = mean(mat_dla_data, dims = 1)
const obs_var = var(mat_dla_data, dims = 1)
const obs_cor = cor(mat_dla_data)
const obs_autocor = autocor(mat_dla_data, 1:L_autocor)

# Sample using block bootstrap method

TT = eltype(mat_dla_data)
...
function unified_metric(
    ...
    return mse_mean, mse_var, mse_autocor, mse_cor
end
...
scatter!(
    ax, [sbb_l], [sbb_mse[sbb_l]],
    label = "SBB $(ft1(100 * beta))% of total decrease",
    markersize = 14,
    color = :blue,
)
scatter!(
    ax, [mbb_l_star], [mbb_min_mse],
    label = "MBB minimum",
    markersize = 14,
    color = :orange
)
...
ax = Axis(fig[1, 1], title = "MSE of covariance matrix")

lines!(ax, 1:length(mbb_stats_metrics[:, 4]), mbb_stats_metrics[:, 4])
lines!(ax, 1:length(sbb_stats_metrics[:, 4]), sbb_stats_metrics[:, 4])
