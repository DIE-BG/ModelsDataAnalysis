...
PLOTSDIR = mkpath(plotsdir("dla_analysis", "autocorrelation", WITH_REMITTANCES ? "with_rem_gdp" : "without_rem_gdp"))
...
obs_autocor = autocor(mat_dla_data, 0:L_autocor)

# Sample using block bootstrap method

# Array of (lags of ACF, variable, block length, bootstrap realization)
sbb_bootstrap_samples_autocor = Array{eltype(mat_dla_data)}(undef, L_autocor + 1, K, L_block, B)
mbb_bootstrap_samples_autocor = Array{eltype(mat_dla_data)}(undef, L_autocor + 1, K, L_block, B)
