...
sbb_bootstrap_samples_cor = Array{eltype(mat_dla_data)}(undef, K, K, L_block, B)
mbb_bootstrap_samples_cor = Array{eltype(mat_dla_data)}(undef, K, K, L_block, B)
...
# Moving
ax = Axis(
    fig[1, 1], xlabel = "Block length", ylabel = "Mean Squared Error",
...
ax = Axis(
    fig[1, 2], xlabel = "Block length", ylabel = "Mean Squared Error",
    title = "Average normalized MSE of lower triangular correlation matrix",
    subtitle = "Stationary"
)
