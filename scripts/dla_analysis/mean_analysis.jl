...
# Matrix data
mat_dla_data = Matrix(DLA_DATA[:, (begin + 1):end]) |> disallowmissing
...
L = size(mat_dla_data, 1) - 1 # Maximum block length is T-1
...
l2means_moving = mapreduce(hcat, 1:L) do l
    ...
    for b in 1:B
        sample_mean[b, :] = mean(mat_dla_data[inds[b], :], dims = 1)
    end
...
## MSE
actual_means = mean(mat_dla_data, dims = 1)
...
var_cod = propertynames(DLA_DATA)[2:end]
...
map(1:nvar) do nvar
    ...
    # MSE moving
    ax = Axis(
        fig[3, 1], title = "Mean square error \nof the historial mean estimator",
        subtitle = "Moving"
    )
    ...
    # MSE stationary
    ax = Axis(
        fig[3, 2], title = "Mean square error \nof the historical mean estimator",
        subtitle = "Stationary"
    )

    lines!(ax, 1:L, l2mse_stationary[:, nvar])

    save(plotsdir(PLOTSDIR) * "\\" * string(nvar) * "_" * string(var_cod[nvar]) * ".png", fig, px_per_unit = 2.0)
...
ax = Axis(
    fig[1, 1], title = "Average MSE of the historial mean estimator",
    subtitle = "Moving",
...
ax = Axis(
    fig[1, 2], title = "Average MSE of the historial mean estimator",
    subtitle = "Stationary",
    xlabel = L"\text{Block length } l"
)
